...
class db_DMFT():
    ...
    def start_solving(sample):
        sample.solve_SC_dmft()
        # print(sample.Giw)
        sys.stdout.flush()
        return sample


    def solve_db(self, n_workers=1):
        ...
        if n_workers > 1:
            ...
            with Manager() as manager:
                ...
                with Pool(n_workers) as pool:
                    results = pool.map(self.start_solving, data_entries)
                    # results = list(tqdm(pool.imap( self.start_solving, data_entries), total=len(data_entries)))
                    # results = list(tqdm(pool.imap(partial(self.start_solving), data_entries),\
    ...
    def plot_SE(self, numV=1, numH=1, what='dens'):
        ...
        for x in range(numV):
            for y in range(numH):
                ...
                sample = self.data_entries[sample_num]
                p = sample.ps_exact[-1]
                SigmaEx = p.sigma_w.data.flatten()
                # print(SigmaEx)
                
                ll = len(SigmaEx)//2
                axes[x, y].plot(np.arange(-ll, ll, 1)*6.28/sample.beta, np.imag(SigmaEx), markersize=8, linewidth=2, label='Exact')
                axes[x, y].legend(loc='best', fontsize=8)
                axes[x, y].set_title(f'U={self.params[sample_num]['U']:.2f} beta={self.params[sample_num]['beta']:.2f}')
                # axes[x, y].set_ylabel(r'$\Sigma(i\omega_n)$')
                axes[x, y].set_xlim([0, 10])
                axes[x, y].set_ylim([-0.5, 0])
                # axes[x, y].set_ylim([-np.min(np.imag(SigmaEx))*1.5, 0.])
                # axes[x, y].set_xlabel(r'$i\omega_n$')
